...
impl<'src, 'e> Checker<'src, 'e> {
    pub fn check_match(
        ...
        let scrut_var = self.create_entity_dummy();
    ...
    fn build_constructor_pattern(
        ...
        match cons {
            ...
            C::Variant(id, tag) => {
                let (_, info) = self.get_union_variant_info(*id, *tag);
                match info.arity() {
                    Some(n) => {
                        let (arg_patterns, arg_ids) = self.create_binding_patterns(n);
    ...
    fn create_binding_patterns(&mut self, n: usize) -> (Vec<ir::Pattern>, Vec<ir::EntityID>) {
        let arg_ids = (0..n)
...
#[derive(Debug, Clone)]
struct MatchTest(ir::EntityID, ir::Pattern);
